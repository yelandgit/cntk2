...
public:
	CPUMatrix(MatrixFormat mft=matrixFormatDenseCol) { Init(mft, CPUDEVICE); }
	CPUMatrix(size_t numRows, size_t numCols, MatrixFormat mft=matrixFormatDenseCol) { Init(mft,CPUDEVICE); Base::Resize(numRows, numCols); }
	CPUMatrix(const CPUMatrix<ElemType>& copyFrom, bool shallow) { Assign(copyFrom, shallow); }

//	CPUMatrix(const CPUMatrix<ElemType>& copyFrom) { SetValue(copyFrom); }
//	CPUMatrix<ElemType>& operator=(const CPUMatrix<ElemType>& copyFrom);

//	CPUMatrix(CPUMatrix<ElemType>&& moveFrom) { Assign(moveFrom, true); moveFrom.Release(); }
//	CPUMatrix<ElemType>& operator=(CPUMatrix<ElemType>&& moveFrom);
...
//
//	void Reshape(size_t numRows, size_t numCols);
//
...
//
//	inline ElemType& operator()(size_t row, size_t col) { return Data()[ItemPos(row, col)]; }
//	inline const ElemType& operator()(size_t row, size_t col) const { return Data()[ItemPos(row, col)]; }
//	inline ElemType Get00Element() const { return operator()(0, 0); }
//
//	void SetValue(ElemType v);
//	void SetValue(const CPUMatrix<ElemType>& copyFrom);
//	//void SetValue(const GPUMatrix<ElemType>& copyFrom);
//	//void SetValue(const CPUSparseMatrix<ElemType>& copyFrom);
//	//void SetValue(const GPUSparseMatrix<ElemType>& copyFrom);
...
//
//	bool IsEqualTo(const CPUMatrix<ElemType>& a, ElemType threshold = 1e-8) const;
//
...
//	static void BatchMatMul(ElemType beta, const CPUMatrix<ElemType>& a, bool transposeA, const int m, const CPUMatrix<ElemType>& b, bool transposeB, const int n, CPUMatrix<ElemType>& c, bool isColWise);
//
...
//
//protected:
//	size_t ItemPos(size_t i, size_t j) const;
//	size_t ColumnPos(size_t j) const;
//
//private:
