...
// max/min
DECL float max(float a, float b) { return fmaxf(a, b); }
DECL half max(half a, half b) { return (float)a > (float)b ? a : b; }
DECL float max(float a, half b) { return a > (float)b ? a : (float)b; }
DECL float max(half a, float b) { return (float)a > b ? (float)a : b; }
DECL float min(float a, float b) { return fminf(a, b); }
DECL half min(half a, half b) { return (float)a < (float)b ? a : b; }
...
DECL float rsqrt_(float v) { return rsqrtf(v); }
DECL double rsqrt_(double v) { return rsqrt(v); }
...
DECL T safepow_(T base, T exponent)
{
	...
	if (base > 0) return pow_(base, exponent);
	int exp_as_int = static_cast<int>(exponent);
	if (exponent != exp_as_int) return T(NAN);
...
{
    return z < EPS_IN_LOG ? (ElemType)LOG_EPS_IN_LOG : log_(z);
}
...
//
// Define a static function for every ElementWiseOperator (BaseMatrix.h).
// -----------------------------------------------------------------------
