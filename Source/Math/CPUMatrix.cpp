...
////{
////	if (&moveFrom != this) { Assign(moveFrom, true); moveFrom.Release(); }
////	return *this;
////}
////
////template <class ElemType>
////CPUMatrix<ElemType>& CPUMatrix<ElemType>::operator=(const CPUMatrix<ElemType>& copyFrom)
////{
////	if (&copyFrom != this) Assign(copyFrom);
////	return *this;
////}
...
CPUMatrix<ElemType>& CPUMatrix<ElemType>::PutColumnSlice(const CPUMatrix<ElemType>& fromMatrix, size_t start, size_t cols)
{
	...
	memcpy(Data() + start*m_numRows, fromMatrix.Data(), cols*m_numRows*sizeof(ElemType));
...
{
	if (IsEmpty()) LogicError("Transpose: Matrix is empty");
...
void CPUMatrix<ElemType>::SetValue(ElemType v)
{
	...
	if (isFinite && v == 0) { memset(Data(), 0, sizeof(ElemType) * GetNumElements()); return; }
...
#pragma omp parallel for
	...
	// handle remaining stuffs
	for (long i = m & ~3; i < m; i++) us(i, j) = val;
}
...
template <class ElemType>
void CPUMatrix<ElemType>::SetValue(const CPUMatrix<ElemType>& copyFrom)
{
	if (&copyFrom != this) SetValue(copyFrom.GetNumRows(), copyFrom.GetNumCols(), copyFrom.Data(), copyFrom.GetFormat());
}
...
void CPUMatrix<ElemType>::SetValue(size_t rows, size_t cols, ElemType* p, int flags)
{
	if (p == nullptr && rows * cols > 0)
		InvalidArgument("Invalid pArray. pArray == nullptr, but matrix is of size %d * %d = %d.", (int)rows, (int)cols, (int)(rows*cols));

	Init(matrixFormatDense, CPUDEVICE);
	if (flags & matrixFlagExternalBuffer) { Assign(rows, cols, p, flags); return; }
	Resize(rows, cols); if (IsEmpty()) return;
	if ((flags & matrixFormatRowMajor)==0) { memcpy(Data(), p, rows*cols*sizeof(ElemType)); return; }
...
#pragma warning(suppress : 4244)
	...
	else RuntimeError("SetValue; Unsupported data format");
}
...
void CPUMatrix<ElemType>::AddGaussianRandomValue(ElemType mean, ElemType sigma, unsigned long seed)
{
	...
	for (long j = 0; j < n; j++)
	{
		...
		// handle remaining stuffs
		for (long i = m & ~3; i < m; i++) us(i, j) = r(generator);
	}
}
...
template <class ElemType>
void CPUMatrix<ElemType>::Reshape(size_t numRows, size_t numCols)
{
...
size_t CPUMatrix<ElemType>::CopyToArray(ElemType*& copyTo, size_t& currSize) const
{
	...
	if (n) memcpy(copyTo, Data(), n*sizeof(ElemType));
	return n;
}
...
template <class ElemType>
inline size_t CPUMatrix<ElemType>::ColumnPos(size_t col) const
{
...
template <class ElemType>
inline size_t CPUMatrix<ElemType>::ItemPos(size_t row, size_t col) const
{
...
{
	if (GetNumElements() == 1) { CPUMatrix<ElemType> c(a); c += (*this)(0, 0); return c; }
	if (a.GetNumElements() == 1) { CPUMatrix<ElemType> c(*this); c += a(0, 0); return c; }
...
{
	if (a.GetNumElements() == 1) { SetValue(b); (*this) += a; }
...
CPUMatrix<ElemType>& CPUMatrix<ElemType>::AssignProductOf(const CPUMatrix<ElemType>& a, bool transposeA, const CPUMatrix<ElemType>& b, bool transposeB)
{
	if (a.GetNumElements() == 1)
	{
		if (transposeB) AssignTransposeOf(b);
		(*this) *= a(0, 0);
	}
	else if (b.GetNumElements() == 1)
	{
		if (transposeA) AssignTransposeOf(a);
		(*this) *= b(0, 0);
	}
	else
...
{
	auto& us = *this;
	if (GetNumElements() == 1) { CPUMatrix<ElemType> c; c.AssignProductOf(us(0, 0), a); return c; }
	if (a.GetNumElements() == 1) { CPUMatrix<ElemType> c; c.AssignProductOf(a(0, 0), us); return c; }
...
CPUMatrix<ElemType>& CPUMatrix<ElemType>::AssignHardmaxOf(const CPUMatrix<ElemType>& a, bool isColWise)
{
	...
	if (!isInplace) memset(us.Data(), 0, a.GetNumElements() * sizeof(ElemType));
...
#pragma warning(suppress : 4244)
	...
	{
		RuntimeError("SumOfAbsElements; Unsupported data format");
	}
}
...
template <class ElemType>
bool CPUMatrix<ElemType>::IsEqualTo(const CPUMatrix<ElemType>& a, ElemType threshold /*= 1e-8*/) const
{
...
#pragma warning(suppress : 4244)
		...
		{
			RuntimeError("VectorNorm2; Unsupported data format");
		}
	}
	else
	{
...
#pragma omp parallel for
	for (int imgIdx = 0; imgIdx < numImg; imgIdx++)
	{
		...
		auto rois = roiData.GetColumnSlice(imgIdx, 1).Data();
		// gradient values for all ROIs from this image. length numRois*pooledHeight*pooledWidth*channels;
		auto pooledGrad = GetColumnSlice(imgIdx, 1).Data();
		auto argmaxCol = argmax.GetColumnSlice(imgIdx, 1).Data();
...
{
		//string s("CPUMatrix");
		//if (std::is_same<ElemType, float>::value) s += "<float>";
		//else if (std::is_same<ElemType, double>::value) s += "<double>";
		//else if (std::is_same<ElemType, half>::value) s += "<half>";
		//else s += "<***>";
		//cout << endl << s << "::MultiplyAndWeightedAdd" << endl;
		//cout << "    a => " << a.str() << endl;
		//cout << "    b => " << b.str() << endl;
		//cout << "    c => " << c.str() << endl;
...
#pragma warning(suppress : 4244)
		...
		{
			RuntimeError("MultiplyAndWeightedAdd; Unsupported data format");
		}
	}
	else
	{
...
void CPUMatrix<ElemType>::ColumnwiseScaleAndWeightedAdd(ElemType alpha, const CPUMatrix<ElemType>& a, const CPUMatrix<ElemType>& v, ElemType beta, CPUMatrix<ElemType>& c)
{
	...
	if (beta == 0) c.Resize(a.GetNumRows(), a.GetNumCols());
	else c.VerifySize(a.GetNumRows(), a.GetNumCols()); // Can't resize if beta != 0
...
void CPUMatrix<ElemType>::SVD(const CPUMatrix<ElemType>& A, CPUMatrix<ElemType>& SIGMA, CPUMatrix<ElemType>& U, CPUMatrix<ElemType>& VT, CPUMatrix<ElemType>& W)
{
	...
	{
		RuntimeError("SVD; Unsupported data format");
	}
#endif
...
#pragma warning(suppress : 4244)
		...
		{
			RuntimeError("ScaleAndAdd; Unsupported data format (1)");
		}
	}
...
#pragma warning(suppress : 4244)
		...
		{
			RuntimeError("ScaleAndAdd; Unsupported data format (2)");
		}
	}
...
#pragma warning(suppress : 4244)
		...
		{
			RuntimeError("ScaleAndAdd; Unsupported data format (3)");
		}
	}
}
...
#pragma warning(suppress : 4244)
	...
	{
		ElemType* p = a.Data();
		for (int j=0; j<len; ++j,++p) *p *= alpha;
	}
	else
	{
		RuntimeError("Scale; Unsupported data format x");
	}
}
...
#pragma warning(suppress : 4244)
		...
		{
			RuntimeError("InnerProduct; Unsupported data format");
		}
	}
	else
	{
...
#pragma warning(suppress : 4244)
	...
	{
		RuntimeError("InnerProductOfMatrices; Unsupported data format");
	}
}
...
template <class ElemType>
void CPUMatrix<ElemType>::BatchMatMul(ElemType beta, const CPUMatrix<ElemType>& a, bool transposeA, const int m, const CPUMatrix<ElemType>& b, bool transposeB, const int n, CPUMatrix<ElemType>& c, bool isColWise)
{
	if (a.IsEmpty() || b.IsEmpty())
	...
	if (beta == 0)
		c.Resize(cSampleElemNum, aBatchSize);
	else
...
void CPUMatrix<ElemType>::InnerProductWithShiftNeg(const CPUMatrix<ElemType>& a, const CPUMatrix<ElemType>& b, CPUMatrix<ElemType>& c, bool isColWise, size_t shift, size_t negnumber)
{
	...
	else if (isColWise) // col-wise
	{
		...
		{
			RuntimeError("InnerProductWithShiftNeg; Unsupported data format");
		}
	}
	else
	{
...
template CPUMatrix<char>::CPUMatrix(size_t numRows, size_t numCols);
template CPUMatrix<char>::CPUMatrix(size_t numRows, size_t numCols, char* pArray, int flags);
...
template CPUMatrix<char> CPUMatrix<char>::GetColumnSlice(size_t startColumn, size_t numCols) const;
template CPUMatrix<char>& CPUMatrix<char>::operator=(CPUMatrix<char>&&);
template void CPUMatrix<char>::SetValue(const char);
template void CPUMatrix<char>::SetValue(size_t numRows, size_t numCols, char* pArray, int flags);
...
//template void CPUMatrix<char>::Resize(size_t numRows, size_t numCols, bool growOnly);
//template void CPUMatrix<char>::Resize(size_t numRows, size_t numCols, bool growOnly);
...
template CPUMatrix<short>::CPUMatrix(size_t numRows, size_t numCols);
template CPUMatrix<short>::CPUMatrix(size_t numRows, size_t numCols, short* pArray, int flags);
...
template CPUMatrix<short> CPUMatrix<short>::GetColumnSlice(size_t startColumn, size_t numCols) const;
template CPUMatrix<short>& CPUMatrix<short>::operator=(CPUMatrix<short>&&);
template void CPUMatrix<short>::SetValue(const short);
template void CPUMatrix<short>::SetValue(size_t numRows, size_t numCols, short* pArray, int flags);
...
//template void CPUMatrix<short>::Resize(size_t numRows, size_t numCols, bool growOnly);
//template void CPUMatrix<short>::Resize(size_t numRows, size_t numCols, bool growOnly);
