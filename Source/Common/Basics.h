...
//==============================================================================
//		ThrowFormatted
//		template function to throw a std::exception with a formatted error string
//==============================================================================

template <class E>
__declspec_noreturn static inline void ThrowFormattedVA(const char* fmt, va_list args)
{
    ...
    {
        buffer = string(size, ' ');
        if (vsnprintf(&buffer[0], size, fmt, args_copy) < 0)
...
{	\
	/*fprintf(stderr, "%s; Line %d; %s --> Feature not implemented\n", __FILE__, __LINE__, __FUNCTION__);*/	\
	LogicError("%s; Line %d; %s --> Feature not implemented", __FILE__, __LINE__, __FUNCTION__);	\
}
#endif
...
///    operator const C*() const { return c_str(); }
///};
...
///            assign(_sprintf(&fixbuf[0], sizeof(fixbuf) / sizeof(*fixbuf), format, args), n);
///        }
///        else // too long: use dynamically allocated variable-size buffer
///        {
///            std::vector<_T> varbuf(n + 1); // incl. '\0'
///            assign(_sprintf(&varbuf[0], varbuf.size(), format, args), n);
///        }
///        va_end(args);
///    }
///
...
///        if (n < 0) perror("The following error occurred in basetypes.h:cprintf");
///        return n;
///#endif
///    }
...
///static inline cstring charpath(const wstring& p)
///{
...
///{
///    vector<std::basic_string<_T>> res;
...
///class tokenizer : public vector<char*>
///{
///    const char* delim;
///
///public:
...
///        for (char* p=strtok_s(buf, delim, &context); p; p = strtok_s(NULL, delim, &context)) push_back(p);
///#else
///        for (char* p = strtok(buf, delim); p; p = strtok(NULL, delim)) push_back(p);
///#endif
///    }
///};
...
///    wstring m_dllName; // name of the writer DLL
///public:
...
///    {
///        return LoadInternal(utf8wc(plugin), proc, isCNTKPlugin);
///    }
///    FARPROC Load(const wstring& plugin, const std::string& proc, bool isCNTKPlugin = true)
///    {
...
///
///private:
///    FARPROC LoadInternal(const wstring& plugin, const std::string& proc, bool isCNTKPlugin);
///};
///#else
///class Plugin
///{
...
///struct ScopeExit {
///    explicit ScopeExit(EF &&f) : m_exitFunction(std::move(f)), m_exitOnDestruction(true) {}
...
///    }
///    ~ScopeExit() { if (m_exitOnDestruction) m_exitFunction(); }
///
///private:
