...
///	fixed_vector(int size) : n(size), p(size > 0 ? new T[size] : NULL) {}
///	fixed_vector(size_t size) : n((int)size), p(size > 0 ? new T[size] : NULL) {}
///	~fixed_vector() { delete[] p; }
///
///	template <class V>
///	fixed_vector(const V &other) : n(0), p(NULL) { *this = other; }
///
///	int  size() const { return (int)n; }
///	void resize(int size) { clear(); if (size > 0) { p = new T[size]; n = size; } }
///	void resize(size_t size) { clear(); if (size > 0) { p = new T[size]; n = size; } }
///	int  capacity() const { return (int)n; }
...
///	T *end() { return p + n; }
///
///	inline T &operator[](int index) { check(index); return p[index]; }
///	inline const T &operator[](int index) const { check(index); return p[index]; }
///	inline T &operator[](size_t index) { check(index); return p[index]; }
///	inline const T &operator[](size_t index) const { check(index); return p[index]; }
///	inline int indexof(const T &elem) const { assert(&elem >= p && &elem < p + n); return &elem - p; }
///	void swap(fixed_vector &other) throw() { std::swap(other.p, p); std::swap(other.n, n); }
///	
///	template <class V>
///	fixed_vector& operator=(const V &other)
///	{
...
///};
///template <class T>
///inline void swap(fixed_vector<T> &L, fixed_vector<T> &R) throw() { L.swap(R); }
...
///	size_t size() const { return fixed_vector<T>::size(); }
///	void resize(size_t n2, size_t m) { numcols = m; fixed_vector<T>::resize(n2 * m); }
...
///};
///template <class _T>
///inline void swap(matrix<_T> &L, matrix<_T> &R) throw() { L.swap(R); }
#endif
...
///	~CCritSec() {}
///
///	void Lock() { m_crsec.lock(); }
///	void Unlock() { m_crsec.unlock(); }
///};
...
///template <class V>
///static inline void byteswap(V &v) throw() { foreach_index(i, v) bytereverse(v[i]); }
...
///		try {
///			body();
///			if (i > 1) fprintf(stderr, "attempt: success after %d retries\n", attempt);
///			break;
///		}
///		catch (const std::exception &e)
///		{
///			if (i >= retries) throw; // failed N times --give up and rethrow the error
...
//template <class S>
//static inline void ZeroStruct(S &s) { memset(&s, 0, sizeof(s)); }
