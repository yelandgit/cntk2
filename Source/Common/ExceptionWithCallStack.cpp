...
static void CollectCallStack(size_t skipLevels, bool makeFunctionNamesStandOut, const function<void(string)>& write)
{
    ...
    if (RtlCaptureStackBackTrace == nullptr) // failed somehow
        return write("Failed to generate CALL STACK. GetProcAddress(\"RtlCaptureStackBackTrace\") failed with error " + 
					utf8c(FormatWin32Error(GetLastError()))  + "\n");

    HANDLE process = GetCurrentProcess();
    if (!SymInitialize(process, nullptr, TRUE))
        return write("Failed to generate CALL STACK. SymInitialize() failed with error " + utf8c(FormatWin32Error(GetLastError())) + "\n");
    ...
    for (size_t i = firstFrame; i < frames; i++)
    {
        ...
        else
        {
            ...
            callStackLine += buf;
            write(callStackLine + " (SymFromAddr() error: " + utf8c(FormatWin32Error(error)) + ")\n");
        }
    }
