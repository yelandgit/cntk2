...
///        if (wantWrite > WRITE_SIZE_LIMIT) wantWrite = WRITE_SIZE_LIMIT;
...
///{
///    long curPos = ftell(f);
///    if (curPos == -1) RuntimeError("error seeking: %s", strerror(errno));
///    int rc = fseek(f, offset, mode);
///    if (rc != 0) RuntimeError("error seeking: %s", strerror(errno));
///    return curPos;
///}
...
///    if (_wstat64(pathname, &fileinfo) == -1) return 0;
...
///void unlinkOrDie(const string& pathname)
///{
...
///}
///void unlinkOrDie(const wstring& pathname)
///{
...
///void renameOrDie(const string& from, const string& to)
///{
///#ifdef _WIN32
...
///void renameOrDie(const wstring& from, const wstring& to)
///{
///#ifdef _WIN32
///    // deleting destination file if exits (to match Linux semantic)
...
///void fputstring(FILE* f, const string& str)
///{
...
///        unique_ptr<char16_t[]> str16(new char16_t[strLen + 1]);
...
///void fputstring(FILE* f, const wstring& str)
///{
///    fputstring(f, str.c_str());
///}
...
///void fgetline(FILE* f, string& s, vector<char>& buf)
///{
///    buf.resize(BUF_SIZE);
...
///void fgetline(FILE* f, wstring& s, vector<wchar_t>& buf)
///{
///    buf.resize(BUF_SIZE);
...
// char buffer version
///void fgetline(FILE* f, vector<char>& buf)
///{
///    buf.resize(BUF_SIZE);
...
///void fgetline(FILE* f, vector<wchar_t>& buf)
///{
///    buf.resize(BUF_SIZE);
///    fgetline(f, &buf[0], (int) buf.size());
...
///void fputfile(const wstring& pathname, const vector<char>& buffer)
///{
...
///void fputfile(const wstring& pathname, const wstring& string)
///{
///    FILE* f = fopenOrDie(pathname, L"wb");
///    try
///    {
...
///void fputfile(const wstring& pathname, const string& string)
///{
///    FILE* f = fopenOrDie(pathname, L"wb");
///    try
///    {
...
///void fgetfile(const wstring& pathname, vector<char>& buffer)
///{
///    FILE* f = fopenOrDie(pathname, L"rb");
///    size_t len = filesize(f);
///    buffer.resize(len);
...
///void fgetfile(FILE* f, vector<char>& buffer)
...
///static size_t fgetfilechars(const wstring& path, vector<char>& buffer)
///{
...
///}
///
///static void fgetfilechars(const wstring& path, vector<char>& buffer, size_t& len)
///{
...
///}
///
///void msra::files::fgetfilelines(const wstring& path, vector<char>& buffer, vector<string>& lines, int numberOfTries)
///{
///    size_t len = 0;
...
///    });
///    // parse into lines
///    vector<char*> lines; lines.reserve(len/20);
///    strtoklines(&buffer[0], lines);
///    return lines;
///}
...
///        {
///            int rc = ::FindClose(h);
///            if ((rc == FINDCLOSE_ERROR) && !uncaught_exception())
///            {
...
///vector<wstring> msra::files::get_all_files_from_directory(const wstring& directory)
///{
...
///#else
///    string d = Microsoft::MSR::CNTK::ToLegacyString(Microsoft::MSR::CNTK::ToUTF8(directory));
...
///        const string fullFileName = d + "/" + fileName;
...
///static inline string wcstombs(const wstring& p) // output: MBCS
///{
///    size_t len = p.length();
...
///    return string(&buf[0]);
///}
///static inline wstring mbstowcs(const string& p) // input: MBCS
///{
///    size_t len = p.length();
...
bool GetDirectory(wstrvec& dset, wstrvec& fset, std::wstring path)
{
	...
	if (patt==L"*" || patt==L"*.*") patt.erase(); else towlower(patt);
	...
	do {
		...
		if (patt.empty() || ComparePath(towlower(info.cFileName),patt)==0) fset.push_back(item);
